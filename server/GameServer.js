...
GameServer.createGame = function(player) {
    ...
    // GameServer.games[thegame.id] = thegame;
    //
    // //Keep track
    // GameServer.count++;
    ...
    // thegame.gamecore = new Core(thegame);
    game = new Core();
    ...
    player.send('s.h.'+ String(game.local_time).replace('.','-'));
...
GameServer.endGame = function(gameid, userid) {
    ...
    if(thegame) {
        ...
        this.log('game removed. there are now ' + GameServer.count + ' games' );
...
GameServer.findGame = function(player) {
    this.log('looking for a game. We have : ' + GameServer.count);
    ...
    if(GameServer.count) {
        ...
        for(var gameid in this.games) {
            ...
            var game_instance = this.games[gameid];
                ...
                GameServer.joinGame(game_instance, player);
...
GameServer.init = function() {
    // Create a new game core instance, this actually runs the
    // game code like collisions and such.
    this.game = new Core(true);

    // Start updating the game loop on the server
    this.game.update(new Date().getTime());
